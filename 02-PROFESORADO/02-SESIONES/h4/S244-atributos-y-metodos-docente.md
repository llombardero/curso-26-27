...
| Resultado de hoy | Separar qué sabe y qué hace una clase. |
| Evidencia mínima | Tabla: clase, atributos, métodos y responsabilidad. |
...
- [ ] Comprobar que la evidencia mínima que se pedirá es: Tabla: clase, atributos, métodos y responsabilidad.
...
Atributos como estado; métodos como comportamiento.
...
| 5–13 min | Explicación breve: Atributos son datos internos; métodos son acciones que puede realizar el objeto. |
| 13–18 min | Demostración o ejemplo: En `Memory`, `memories` es atributo; `remember` es método. |
| 18–35 min | Trabajo del alumnado: Diseñar tarjeta CRC simple de `Agent` y `Memory`. |
| 35–40 min | Comprobar la evidencia con una explicación o prueba breve. |
| 40–45 min | Cierre: Pregunta: ¿qué sabe `Memory` y qué hace `Memory`? |

## Ejemplo o demostración preparada

En `Memory`, `memories` es atributo; `remember` es método.

## Consigna que se entrega al alumnado

Tabla de atributos y métodos de `Agent` y `Memory`.

Producto o evidencia que debe quedar: **Tabla: clase, atributos, métodos y responsabilidad.**
...
Pregunta de control: **¿Qué sabe `Agent`? ¿Qué hace `Agent`?**
