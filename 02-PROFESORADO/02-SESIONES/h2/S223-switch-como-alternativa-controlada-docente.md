...
Diferencia entre `while`, `do-while` y `for`; condición de salida; riesgo de bucle infinito; uso razonado de `break` y `continue`; eficiencia básica.
...
| 5–13 min | Explicación breve: `switch` puede ser más legible cuando hay muchas opciones exactas. |
| 13–18 min | Demostración o ejemplo: Reescribir dos comandos con `switch`. |
| 18–35 min | Trabajo del alumnado: Resolver tres mini-ejercicios: pedir una opción al menos una vez con `do-while`, repetir 5 mensajes con `for` y comparar una búsqueda simple con una búsqueda repetida dentro de otro bucle. |
| 35–40 min | Comprobar la evidencia con una explicación o prueba breve. |
| 40–45 min | Cierre: Pregunta: ¿cuándo elegirías `for` en vez de `while`? |

## Ejemplo o demostración preparada

Reescribir dos comandos con `switch`.
