...
- [ ] Comprobar que la evidencia mínima que se pedirá es: Primera herramienta implementa `Tool`.
...
Interfaz como conjunto de métodos que una clase promete implementar.
...
| 5–13 min | Explicación breve: Una interfaz define un contrato común. |
| 13–18 min | Demostración o ejemplo: `Tool` con `name`, `description`, `execute`. |
| 18–35 min | Trabajo del alumnado: Crear `Tool` con métodos simples como `name()` y `execute()`. |
| 35–40 min | Comprobar la evidencia con una explicación o prueba breve. |
| 40–45 min | Cierre: Microdefensa: ¿qué obliga a hacer la interfaz? |

## Ejemplo o demostración preparada

`Tool` con `name`, `description`, `execute`.

## Consigna que se entrega al alumnado

Crear primera implementación.

Producto o evidencia que debe quedar: **Primera herramienta implementa `Tool`.**
...
Pregunta de control: **¿Qué obliga a implementar una interfaz?**
