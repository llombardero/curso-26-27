...
- [ ] Comprobar que la evidencia mínima que se pedirá es: Comandos `recuerda`, `memoria` y `estado` siguen funcionando.
...
| 5–13 min | Explicación breve: Extraer clase significa mover responsabilidad sin cambiar comportamiento externo. |
| 13–18 min | Demostración o ejemplo: Pasar `memories.add(text)` a `memory.remember(text)`. |
| 18–35 min | Trabajo del alumnado: Mover operaciones de memoria a la clase `Memory`. |
| 35–40 min | Comprobar la evidencia con una explicación o prueba breve. |
| 40–45 min | Cierre: Prueba de regresión manual. |

## Ejemplo o demostración preparada

Pasar `memories.add(text)` a `memory.remember(text)`.

## Consigna que se entrega al alumnado

Extraer operaciones de memoria.

Producto o evidencia que debe quedar: **Comandos `recuerda`, `memoria` y `estado` siguen funcionando.**
...
Pregunta de control: **¿Qué prueba demuestra que no has roto H3?**
