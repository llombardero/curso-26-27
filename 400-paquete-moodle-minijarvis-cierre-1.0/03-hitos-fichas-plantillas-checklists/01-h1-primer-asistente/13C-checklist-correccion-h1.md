# Checklist de corrección — H1 Primer asistente básico

## Programación + Entornos de Desarrollo — 1.º DAW — Curso 2026/2027

Documento para uso docente.

Relacionado con:

- `13-guia-docente-h1-primer-asistente.md`
- `13B-ficha-alumnado-h1-primer-asistente.md`
- `../../06-rubricas-hitos.md`

---

## 1. Datos de la entrega

Alumno/a o equipo:

```text

```

Fecha:

```text

```

Repositorio o ubicación:

```text

```

---

## 2. Mínimos técnicos

| Ítem | Sí | Parcial | No | Observaciones |
|---|---|---|---|---|
| Existe `src/Main.java` | | | | |
| La clase principal se llama `Main` | | | | |
| Existe método `public static void main(String[] args)` | | | | |
| El programa compila o se ejecuta en IntelliJ | | | | |
| Muestra un saludo inicial | | | | |
| Pide el nombre de la persona usuaria | | | | |
| Guarda el nombre en una variable | | | | |
| Usa al menos una constante con `final` | | | | |
| Muestra mensajes relacionados con MiniJarvis | | | | |
| Cierra o gestiona razonablemente el `Scanner` | | | | |

---

## 3. Ajuste al hito

| Ítem | Sí | Parcial | No | Observaciones |
|---|---|---|---|---|
| No incluye menú | | | | |
| No incluye bucles | | | | |
| No incluye `switch` | | | | |
| No incluye memoria, listas o mapas | | | | |
| No incluye IA real | | | | |
| No incluye clases avanzadas innecesarias | | | | |
| El código es simple y adecuado a H1 | | | | |

Si hay complejidad extra, anotar:

```text
¿Es defendible?
¿Fue pedida por el docente?
¿Oculta falta de comprensión básica?
```

---

## 4. Código limpio inicial

| Ítem | Sí | Parcial | No | Observaciones |
|---|---|---|---|---|
| Nombres de variables comprensibles | | | | |
| Nombres de constantes comprensibles | | | | |
| Mensajes claros para el usuario | | | | |
| Código legible y ordenado | | | | |
| No hay duplicación o ruido innecesario | | | | |
| Cada línea importante puede explicarse | | | | |

---

## 5. Entornos de Desarrollo

| Ítem | Sí | Parcial | No | Observaciones |
|---|---|---|---|---|
| Proyecto abierto/configurado en IntelliJ | | | | |
| El alumno/a sabe ejecutar desde IntelliJ | | | | |
| README presente | | | | |
| README explica qué hace el programa | | | | |
| README explica cómo ejecutar | | | | |
| Incluye ejemplo o evidencia de ejecución | | | | |
| Repositorio o entrega organizada | | | | |

---

## 6. Uso de IA

| Ítem | Sí | Parcial | No / No aplica | Observaciones |
|---|---|---|---|---|
| Declara si ha usado IA | | | | |
| Incluye registro IA si procede | | | | |
| Explica para qué la usó | | | | |
| Indica qué aceptó o modificó | | | | |
| Verificó el resultado | | | | |
| Puede defender el código asistido | | | | |
| No hay uso oculto evidente | | | | |

Señal de alerta:

```text
Código muy superior al nivel H1 + incapacidad de explicarlo.
```

---

## 7. Preguntas de defensa

Marcar las que se han preguntado y anotar resultado.

| Pregunta | Respuesta adecuada | Dudas | No responde | Observaciones |
|---|---|---|---|---|
| ¿Dónde empieza el programa? | | | | |
| ¿Qué hace `main`? | | | | |
| ¿Qué variable guarda el nombre? | | | | |
| ¿Qué constante has usado? | | | | |
| ¿Qué hace `Scanner`? | | | | |
| ¿Qué hace `nextLine()`? | | | | |
| ¿Qué diferencia hay entre `print` y `println`? | | | | |
| ¿Cómo ejecutas desde IntelliJ? | | | | |
| ¿Por qué todavía no hay menú? | | | | |
| ¿Has usado IA? ¿Cómo lo verificaste? | | | | |

---

## 8. Valoración rápida por dimensiones

| Dimensión | 4 Excelente | 3 Adecuado | 2 Básico | 1 Insuficiente | Nota/observación |
|---|---|---|---|---|---|
| Programa Java básico | | | | | |
| Ajuste al nivel H1 | | | | | |
| IntelliJ y ejecución | | | | | |
| README | | | | | |
| Defensa | | | | | |
| IA responsable, si procede | | | | | |

---

## 9. Decisión docente

Estado de la evidencia:

```text
[ ] Válida.
[ ] Válida con mejoras menores.
[ ] Necesita recuperación parcial.
[ ] No válida todavía.
```

Mejoras o recuperación solicitada:

```text

```

Fecha de revisión:

```text

```

---

## 10. Observación final

```text

```

<!-- AJUSTE-COBERTURA-CONCEPTOS-32 -->

## Ajuste curricular — cobertura de conceptos de Programación

Este hito queda alineado con el mapa `32-lista-conceptos-programacion-por-tema.md`.

```text
Hito: H1
Temas de referencia: Tema 1
Foco: primer programa Java por consola y asistente mínimo
```

Conceptos que deben trabajarse o, como mínimo, quedar conectados con evidencias del alumnado:

- programa
- instrucción
- sintaxis
- comentarios
- variable
- tipo de dato
- literal
- String
- Scanner
- entrada/salida
- operadores
- expresiones
- conversiones básicas

Criterio docente de cierre:

- El alumnado no solo entrega el producto; debe poder señalar dónde aparece cada concepto en su código, README, pruebas o defensa.
- Si un concepto se marca como ampliación, no penaliza al alumnado que alcance el mínimo, pero sí orienta mejora, recuperación o enriquecimiento.
- La defensa debe incluir al menos una pregunta de comprensión sobre los conceptos nuevos del hito.

