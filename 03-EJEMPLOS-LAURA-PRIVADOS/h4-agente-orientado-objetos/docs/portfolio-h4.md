...
```text
He separado el código en Main, Agent y Memory. También he creado el diagrama de clases y he relacionado el diagrama con el código.
```

## Qué he aprendido sobre POO

```text
He aprendido que una clase debe tener una responsabilidad clara. Main no debe saber todos los detalles del agente, y Memory no debe saber cómo funciona el menú.
```

## Qué clase puedo defender

```text
Puedo defender Memory porque tiene un atributo privado ArrayList, un constructor, un método add, un método count y un método formatMemories.
```
