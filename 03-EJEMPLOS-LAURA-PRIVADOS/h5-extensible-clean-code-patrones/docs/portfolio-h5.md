# Portfolio individual — H5

## Qué he hecho yo

```text
He separado los comandos en herramientas, he añadido CourseTool y he documentado por qué usamos Tool como Command simplificado.
```

## Qué he aprendido sobre código limpio

```text
He aprendido que refactorizar no es solo cambiar nombres. Refactorizar debe mejorar el diseño sin romper el comportamiento.
```

## Qué puedo defender

```text
Puedo defender Tool, CourseTool, registerDefaultTools y por qué Agent ya no necesita un if/else para cada comando.
```
